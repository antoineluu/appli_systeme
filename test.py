...
if cfg.weightfile is None:
    ...
    # cfg.weightfile = './snapshot/BaggingECELMs/RealPE/2021/weights/8CELMs.pth.tar'
...
# X, ca, cr = readsamples(fileTest, keys=keys, nsamples=[100], groups=[25], mode=modeTest, parts=None, seed=seed)
# th.save(X,'./tensors/Xtest.pt')
# th.save(ca,'./tensors/ca.pt')
# th.save(cr,'./tensors/cr.pt')
X= th.load('./tensors/Xtest.pt')
ca= th.load('./tensors/ca.pt')
cr= th.load('./tensors/cr.pt')
N, Na, Nr, _ = X.size()
...
net.plot(X, ca, cr, xa, index, 'perf_64', outfolder, device)

vars=[1e-1,1,2,1e1]
for var in vars:


    X_speckly = speckle(X,var)
    lossvtest = net.ensemble_test(X_speckly, ca, cr, size_batch, loss_ent_func, loss_cts_func, loss_fro_func, device, name='Test')
    net.plot(X_speckly, ca, cr, xa, index, 'noise_speckle_'+str(var), outfolder, device)

    X_gaussian= gaussian(X,var)
    lossvtest = net.ensemble_test(X_gaussian, ca, cr, size_batch, loss_ent_func, loss_cts_func, loss_fro_func, device, name='Test')
    net.plot(X_gaussian, ca, cr, xa, index, 'noise_gaussian_'+str(var), outfolder, device)

    X_additive = additive_noise(X,var)
    lossvtest = net.ensemble_test(X_additive, ca, cr, size_batch, loss_ent_func, loss_cts_func, loss_fro_func, device, name='Test')
    net.plot(X_additive, ca, cr, xa, index, 'noise_additive_'+str(var), outfolder, device)
