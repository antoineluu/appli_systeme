...
class BaggingECELMs(th.nn.Module):
    ...
    def train_valid(self, Xtrain, catrain, crtrain, Xvalid, cavalid, crvalid, sizeBatch, nsamples1, loss_ent_fn, loss_cts_fn, loss_fro_fn, Cs, device):
        # apprentissage et tuning des beta pour chaque tradef-off coefficient sur les données de validations
        tstart = time.time()
        bestC = [0.] * self.Ncelms
        with th.no_grad():
            for n in range(self.Ncelms):
    ...
    def ensemble_test(self, X, ca, cr, sizeBatch, loss_ent_fn, loss_cts_fn, loss_fro_fn, device, name='Test'):
        # inférence sur le ensemble model, sur la data X et calcul de la loss à partir de des coefficients.
        self.eval()
        if self.cstrategy == 'averagephase':
        ...
        with th.no_grad():
            ...
            print("--->" + name + " ensemble: %d, loss: %.4f, entropy: %.4f, entropy_x: %.4f, contrast: %.4f, time: %ss" %
                  (self.Ncelms, lossvtest, lossENTv, lossentx, lossCTSv, tend - tstart))
        return lossvtest
